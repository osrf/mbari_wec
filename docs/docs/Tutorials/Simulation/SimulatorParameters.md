...
- **Spring Params**:  The buoy implements an air spring component with two N2-filled chambers (upper and lower). These are modeled in the sim by a combo of polytropic curves and heat transfer. Hysteresis is modeled by two different polytropic indices per chamber.
